describe('blog app', function() {
  ...
  describe('when logged in', function() {
    ...
    it('5.19 a blog can be created', function() {
      ...
      cy.get('#listOfBlogs')
        .should('contain', 'a new blog from cypress')
    })
    ...
    it('5.21 other users cannot delete the blog', function() {
      cy.get('#logout').click()
      cy.loginUser({
        username: 'shizuka123',
        password: '123456789'
    ...
    it('5.22 blogs are sorted by likes in descending order', function() {
      cy.get('.view').click({ multiple: true })
      cy.get('.blog').then(blogs => {
        const pattern = /(?<=likes: )[0-9]+/
        const likesArray = blogs.map((index, blog) => parseInt(blog.innerText.match(pattern)[0]))
