...
test('5.13 blog does not render its url or likes by default', () => {
  expect(component.getByText(`${testBlog.title} by ${testBlog.author}`)).not.toBe(null)
...
test('5.14 blog shows its url and likes when the view button has been clicked', () => {
...
test('5.15 the like button is clicked twice', () => {
  const button = component.getByText('view')
  fireEvent.click(button)
